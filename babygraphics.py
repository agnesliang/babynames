...
def index_to_x(width, year_index):
    ...
    """
    sub_width = (width - (SPACE * 2)) // len(YEARS)
    x = SPACE + (sub_width * year_index)
    return x
...
def draw_fixed(canvas):
    ...
    for i in range(len(YEARS)):
        ...
        canvas.create_text(x + TEXT_DX, (height-SPACE) + TEXT_DX, text=str(YEARS[i]), anchor=tkinter.NW)
...
def best_rank(names, name, year):
    ...
    1000
    """
    if name not in names or year not in names[name]:
        return MAX_RANK
    return names[name][year]
...
#1:
    ...
    for i in range(len(YEARS)):
        x = index_to_x(width, i)
        y = SPACE + (best_rank(names, name, YEARS[i]) / MAX_RANK) * (height - (SPACE * 2))
        canvas.create_line(x, y, x + 40, y, width=LINE_WIDTH, fill='red')

#2:Text
        t_name = name + '' + str(best_rank(names, name, YEARS[i]))
        canvas.create_text(x + TEXT_DX, y, text=t_name, anchor=tkinter.SW, fill='red')
...
def draw_names(canvas, names, lookups):
    ...
    for index in range(len(lookups)):
        name = lookups[index]
        if index < len(COLORS):
            draw_name(canvas, names, name, COLORS[index])
